...
func TestDetect_AuroraWriter_FromBasedir(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to create mock: %v", err)
	}
	...
	// Mock VERSION() — real Aurora returns standard MySQL compat version
	mock.ExpectQuery("SELECT VERSION\\(\\)").
	...
	// Mock basedir — Aurora marker (real Aurora basedir)
	mock.ExpectQuery("SHOW GLOBAL VARIABLES LIKE 'basedir'").
...
func TestDetect_AuroraReader_FromBasedir(t *testing.T) {
	db, mock, err := sqlmock.New()
	if err != nil {
		t.Fatalf("failed to create mock: %v", err)
	}
